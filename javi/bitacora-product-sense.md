...
### 103. "Karina también es co-founder — y nada de nombres: FV, KM, DG, JC"
- **Contexto:** revisión de la página de nosotros tras el sprint de planes.
- **Petición:** *"el diseño no me está gustando nada, no estás tomando los MD de UI/UX de todo eso que estudiamos y tenemos guardado. Y otra cosa: Karina también es co-founder. Y sobre los nombres, solo usaremos la inicial — nada de nombres. Inicial de nombre y apellido: FV, KM, DG, JC."*
- **Razonamiento:** tres capas. (1) El curso es LA vara y me cachó violándola: todas las cards de contenido iban en glass cuando la Parte C dice "el vidrio es el chrome, jamás el contenido", y había cards no-clickeables levantándose al hover — releer la sección ANTES de construir no es opcional. (2) La corrección societaria importa: los 4 son co-founders, la mesa pareja se refleja hasta en el título de cada card. (3) Las iniciales son una decisión de privacidad deliberada: la cara pública del equipo son sus responsabilidades (CSO/COO/CPO/CTO y su pregunta), no sus identidades — coherente con una empresa que aún está formalizando acta y marca.
- **Término(s):** el curso se consulta ANTES, no después del regaño · los 4 son co-founders parejos · en público: iniciales y responsabilidades, jamás nombres.

### 104. "Hazme algo excelente — página-experiencia, no columna genérica"
- **Contexto:** el pulido de /nosotros (sólido + iniciales) le supo a poco.
- **Petición:** *"No me termina de gustar, la veo muy genérica para ser una página de about us. Le falta diseño, experiencia, algo visual agradable, tecnológico, animaciones, movimientos, transiciones, cosas diferentes — no limitarnos a la columna de en medio. Usa cosas de React, animaciones de carrusel… [ReactBits] tiene text animations, animations, componentes, backgrounds — analiza cada uno de todos esos y hazme algo excelente… usa psicología de layouts, todo eso."*
- **Razonamiento:** distinción clave que me enseñó: cumplir el curso ≠ tener diseño. La versión sólida era CORRECTA (tokens, hover honesto) pero seguía siendo una columna central de cards — corrección sin ambición. Para una página de marca, el estándar es EXPERIENCIA: ritmo de bandas full-bleed, cifras que cuentan solas, historia que se recorre, marca que brilla. Y su método es curatorial: no "ponle animaciones" sino "analiza un catálogo (ReactBits) y elige con criterio" — la curaduría se implementó en motor propio de la casa (sin dependencias nuevas) y dentro del presupuesto del curso.
- **Término(s):** correcto no es excelente — las páginas de marca se diseñan como experiencia · romper la columna: el layout también comunica · catálogos como ReactBits se analizan y se implementan en casa, no se copian.

### 105. "No sabes cómo regresar al home — yo me di cuenta después de horas"
- **Contexto:** probando el login con la cuenta de prueba del encendido.
- **Petición:** *"cuando entras aquí no sabes cómo regresar atrás o al home, no hay nada — solo el nombre de Enkoras ahí, pero no se ve que sea clickeable; yo me di cuenta después de horas de buscar. Y no sé si la parte de color negro sea el mejor diseño… no sé si aquí hemos aplicado lo de ReactBits o los MD de UI/UX."*
- **Razonamiento:** lección de affordance de libro: un link que no PARECE link no existe — y el costo lo midió él mismo: horas. En una puerta (login/registro) la salida es parte del diseño: el usuario que llegó por error o quiere ver la página antes de registrarse necesita el regreso donde lo espera (arriba a la izquierda, con flecha y texto), no escondido en un logo mudo. Y su instinto sobre el panel negro no era "quítalo" sino "está muerto": la respuesta correcta fue darle la vida del lenguaje nuevo (aurora + órbitas + halo), no cambiarle el color — la tinta es marca.
- **Término(s):** lo clickeable se tiene que VER clickeable · toda puerta necesita salida visible donde se espera · "no me convence" a veces significa "está muerto", no "cámbialo".

### 106. "Auditoría profunda nivel senior para YA liberar la página"
- **Contexto:** con todo construido y el cobro encendido, pidió el barrido final.
- **Petición:** *"lánzate una auditoría completa del backend y el frontend por favor, backend profundo, para ya liberar la página… me preocupa que haya errores de código o que la página se me llegue a caer cuando ya tenga 10 users o menos… cazar todo lo que se nos haya perdido o escondido mientras revisábamos. Una auditoría nivel senior experto."*
- **Razonamiento:** el instinto correcto en el momento correcto: auditar DESPUÉS del encendido, porque prender el flag cambió el mundo (los candados operando de verdad, el cupón real, la suite asumiendo el flag apagado). 4 auditores en paralelo con mandato de evidencia-o-nada; resultado: CERO S1 (su miedo de la caída con 10 users, descartado con mediciones), 8 S2 verificados y cerrados el mismo día — incluida la joya: el cupón 100% que él estaba a punto de probar no activaba el plan. La lección de método: cada capa nueva se audita ANTES de que la pise un usuario, y el "mundo cambió" (flag ON) invalida fixtures y supuestos viejos.
- **Término(s):** auditar después de cada encendido — el flag cambia el mundo · cero S1 se demuestra con mediciones, no con confianza · lo recién construido es lo menos auditado de la casa.
